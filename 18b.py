...
for bytes in range(0, len(memory)):
  print("Doing " + str(bytes) + ' / ' + str(len(memory)));
  last_x = -1;
  last_y = -1;
  dist_map = []
  count = -1;
  ...
  #print("Result: " + str(dist_map[max_y+1][max_x+1]));
  if dist_map[max_y+1][max_x+1] == startval:
    print("PART B:")
    print(str(last_x-1) + ',' + str(last_y-1));
    exit();
